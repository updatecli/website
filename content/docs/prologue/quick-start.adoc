...
1. To see what would change: `updatecli diff --config updatecli.yaml`
2. To apply the change: `updatecli apply --config updatecli.yaml`
3. To see that nothing needs to be changed anymore: `updatecli apply --config pipeline.yaml`
