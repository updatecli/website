...
<p>This release fix a major issue with the GitHub scm plugin, using set setting <strong>commitusingapi</strong> set to true, where an Updatecli manifest containing multiple targets would create wrong commits. This is fixed now, which allows us to move this feature out of experimental. Depending on how stable this feature is, we will consider turning on this setting by default for the GitHub SCM plugin as it increases the security of git project.</p>
