---
title: "What a year 2025 has been! See you in 2026!"
date: 2025-12-31T00:00:00+00:00
draft: false
weight: 50
images: [""]
contributors: ["olblak"]
---

It's time to close another great year on the Updatecli journey.

This year we achieved 29 releases contributed by over 20 community members.

It wouldn't be possible to list all the improvements in this post, so feel free to take a look to our website for a detailed list of changes on [**updatecli.io**](https://www.updatecli.io/changelogs/updatecli/)

And not only did we continue consolidating and improving Updatecli, we also experimented with new ideas.

<br/><br/>

---

##### External Plugin Ecosystem

For example, defining update strategies in YAML is convenient, saves time, and helps teams collaborate. But when update rules grow complex, a small DSL can become limiting. This year we explored using WASM to let teams write manifest generators in the language they prefer. The feature is still experimental, and we welcome feedback from teams with complex update scenarios.

More information [**here**](https://github.com/updatecli/updatecli/pull/7034)

---
<br/><br/>

This year was also special for conferences and community events. A heartfelt thank-you to Giovanni Toraldo and Anthony Benguerfi for presenting and sharing their experiences, it was inspiring to watch.

<br/><br/>
{{< youtube kVOYFFbbCho >}}
<br/><br/>

We presented talks at FOSDEM, Open Source Experience, and Incontro DevOps (in English, Italian, and French).

Conference & talk:

- FOSDEM - Olivier Vernin - [**recording**](https://archive.fosdem.org/2025/schedule/event/fosdem-2025-6076-continuously-update-everything-two-years-later/)
- Incontro DevOps - Giovanni Toraldo - [**recording**](https://www.youtube.com/watch?v=kVOYFFbbCho)
- Open Source Experience - Anthony Benguerfi — recording forthcoming; follow [**Open Source Experience**](https://www.youtube.com/@opensourceexperience7693)

Selected community write-ups:

- IaC drift resolution (Loïs Postula) - [**link**](https://lois.postu.la/posts/iac-drift-resolution-using-updatecli/)
- Automating tooling upgrades (Sequra Tech) - [**link**](https://medium.com/sequra-tech/automating-tooling-upgrades-with-updatecli-a-scalable-solution-for-platform-teams-c9f599e301dc)
- Automating Ruby version upgrades (Sequra Tech) - [**link**](https://medium.com/sequra-tech/automating-ruby-version-upgrades-f71d19e26aeb)

Thanks [**Prefect**](https://www.prefect.io/) for sponsoring us one more year!

<br/><br/>
<p align="center">
  <img src="/images/sponsors/prefect.png" alt="Prefect Logo"/>
</p>
<br/><br/>

## Next Year

What’s coming in 2026? While we can't predict everything, we already plan to be at [**FOSDEM**](https://fosdem.org/2026/) and [**Configuration Management Camp**](https://cfgmgmtcamp.org/), so please come say hi if you're attending.

Our goals for 2026 include:

- Continue maturing plugin extensibility.
- Improve discovery and scaffolding for common project types.
- Increase the number of sponsors to help us fund development and community activities.

We'd love more community blog posts and talks, if you'd like help preparing a talk or post, reach out. Thanks to our sponsors, we can provide swag for contributors and speakers.

<br/><br/>
<p
  align="center"
  style="font-size: 1.2em; font-weight: bold;"
>
  Happy new year! See you in 2026!
</p>
<br/><br/>
