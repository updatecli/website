...
Only tasks using the `docker` driver are considered, since the image is read from the driver's `config.image`. Tasks using `exec`, `raw_exec`, `java`, or any other driver are ignored.
...
Each image produces a link:{{< ref "docker_image" >}}[`dockerimage`] source for the latest tag and an link:{{< ref "hcl" >}}[`hcl`] target that rewrites `config.image` in place. When digest pinning is enabled, a link:{{< ref "docker_digest" >}}[`dockerdigest`] source is added and the digest is written alongside the tag.
