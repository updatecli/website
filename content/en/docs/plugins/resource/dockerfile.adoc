---
title: "Dockerfile"
description: "Read and update instructions in a Dockerfile, by text matching or by coordinates"
...
menu:
  docs:
    identifier: "resource-dockerfile"
...
The Dockerfile "source" reads the value of a Dockerfile instruction and returns it, so one Dockerfile can drive updates elsewhere.
...
=== Reading a value with a source

A source takes exactly one file. More than one entry in `files` fails with `the attributes 'spec.files' can't contain more than one element for sources`, and an empty spec with `no dockerfile specified`.
...
[source,shell]
----
$ updatecli diff --config source.yaml
✔ value "1.15.8" found for last stage in the dockerfile file "Dockerfile.webapp"
----

`stage` narrows the search to one named build stage; without it the **last** stage is used, which the report states explicitly:
...
[source,shell]
----
$ updatecli diff --config source-stage.yaml
✔ value "golang:${GO_VERSION}-alpine" found for stage "build" in the dockerfile file "Dockerfile.webapp"
----
...
| file  | &#10004; (if `files` is not set)  | - | Specifies the Dockerfile path to use and is incompatible with `files`
| files  | &#10004; (if `file` is not set) | - | Specifies the Dockerfile paths to use and is incompatible with `file`. A source accepts only one entry.
| instruction.keyword | &#10004; | - | Set the <<Supported Keywords>> to be matched.
| instruction.matcher | &#10004; | - | Set the text to be matched.
| stage | &#10007; | last stage | Restricts matching to one named build stage (the name after `AS`). Unset means the last stage.
|===
