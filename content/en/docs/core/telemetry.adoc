...
| Extra headers sent to the collector, as a comma separated list of `key=value` pairs. This is how authenticated backends such as Honeycomb or Grafana Cloud are configured. `OTEL_EXPORTER_OTLP_TRACES_HEADERS` overrides it for traces only.

| `OTEL_EXPORTER_OTLP_INSECURE`
| Set to `true` to talk to a collector over plaintext gRPC.
|===

Only `OTEL_TRACES_EXPORTER` and the endpoint variables are read by Updatecli itself.
The others are interpreted by the OpenTelemetry Go SDK, so the full set documented in the
https://opentelemetry.io/docs/specs/otel/protocol/exporter/[OTLP exporter specification] applies —
including timeouts, compression, and TLS certificates.
...
* An unrecognized `OTEL_TRACES_EXPORTER` value does not fail the run: Updatecli logs a warning and disables tracing.
* The service name reported to the backend is always `updatecli`, and the service version is the Updatecli version.
====
