...
CI job rather than on every invocation. An unparsable boolean value is ignored and logged at debug level.
An invalid `UPDATECLI_RELATIVE_PATHS` value is not ignored: the run fails with
`invalid manifest options: "<value>" is not a valid value for "relativepaths"`.
