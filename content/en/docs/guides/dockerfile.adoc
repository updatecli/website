...
description: "How the Jenkins project keeps its Dockerfiles, YAML, and HCL files on the same JDK version"
...
* `updatecli manifest show --config updatecli/updatecli.d/jdk17.yaml --values updatecli/values.yaml --experimental --graph --graph-flavor=mermaid`
...
updatecli pipeline diff --config updatecli/updatecli.d/jdk17.yaml --values updatecli/values.yaml
----
...
The `scm` section accepts different kinds of configuration such as `git`, `gitea`, `gitlab`, etc.

In the current scenario, we need to update files on a GitHub repository so we use the scm of kind `github`.
We could also have used the kind `git`, but as you'll discover later in this guide, we need to leverage specific GitHub behaviour such as GitHub pull request.


Worth mentioning, most of the time we keep the scm settings parametrised so we can easily override all pipeline values. For example, to test the workflow against a different Git repository.

For that we use a file named `values.yaml` with different settings.
The `token` key holds the *name* of the environment variable to read, which is why the manifest uses `requiredEnv .github.token` rather than a hardcoded variable name.
...
* Dry run mode: `updatecli pipeline diff --config updatecli/updatecli.d/jdk17.yaml --values updatecli/values.yaml`
* Apply mode: `updatecli pipeline apply --config updatecli/updatecli.d/jdk17.yaml --values updatecli/values.yaml`
...
The `sources` section defines how to retrieve `source` information. There are many different kinds of `source` like `dockerimage`, `dockerdigest`, etc, where each plugin relies on different parameters specified within the `spec` section.
In this case we decide to use a source of kind `temurin` which allows us to retrieve a specific Temurin version.

The first step here is to identify the "latest" version for Java 17.
...
Once again, there are many things we would like to test, in this case we want to be sure that the Temurin version is available for all the architectures we need to build a Docker image for.
...
We can have as many targets as we want. Our manifest contains six targets, but only three of them are detailed here.

Each target with the same scmid will create one Git commit, all of them landing in the same pull request as defined later by our action.
...
The `setJDK17VersionWindowsDockerImage` target updates the Dockerfile ARG JAVA_VERSION for the Windows Server Core image.
...
The `setJDK17VersionWindowsDockerCompose` target updates the JDK17 version in the `build-windows.yaml` file. Instead of using the plugin `dockerfile`, we use the plugin `yaml` to update the version number in the `build-windows.yaml` file.
...
The `setJDK17VersionDockerBake` target updates the JDK17 version in the `docker-bake.hcl` file consumed by Docker Buildx Bake. Instead of using the plugin `dockerfile`, we use the plugin `hcl` to update the version number in the `docker-bake.hcl` file.
...
Once again there are many different kinds of action but in this case we are leveraging the action of kind `github/pullrequest` that must be bound to a scm using the parameter `scmid`.
So we are telling Updatecli to open a pull request on the GitHub repository link:https://github.com/jenkinsci/docker[jenkinsci/docker] and to assign the labels `dependencies` and `jdk17`.

It is worth mentioning that by default, the GitHub scm will do all its operations from a temporary Git branch named `updatecli_main_xxx`, so the pull request will be opened from this branch.
...
* link:/docs/automate/jenkins/[in the corresponding Jenkins documentation]
* link:/docs/automate/github_action/[in the corresponding GitHub Action documentation]
